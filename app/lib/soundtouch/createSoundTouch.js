    ...
    class SoundTouchNode extends AudioWorkletNode {
        ...
        makeCallback(time) {
            return new Promise((resolve) => {
                this.on('play', (detail) => {
                    console.log(detail);
                    if (detail.sourcePosition > time) {
                        this.off('play');
                        resolve(detail.sourcePosition);
                    }
                })
            })
        }
        ...
        cutCallback(cb) {
            if (cb) {
                this._cutCallback = cb;
            } else if (this._cutCallback) {
                console.log("CUT");
                this._cutCallback();
            }
        }
        ...
        disconnectFromBuffer() {
            console.log(">>>disconnectFromBuffer");
        ...
        handleAudioData() {
            ...
            this.port.postMessage({
                ...
                detail: this.createBaseArray(this.audioBuffer),
            });
        }
        ...
        _messageProcessor(eventFromWorker) {
            ...
            //console.log(">>>messageProcessor" + detail);
            // if (message === "LOG")
            //     console.log(detail);
